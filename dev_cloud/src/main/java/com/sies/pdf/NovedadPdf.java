...
public class NovedadPdf {
   ...
   public  static void generarPdfNovedades(String filename, Collection novedades,  UsuarioBasico medico )
   {
       ...
       String[] reportHeader = {
		...
		if(System.getProperty("file.separator").equals("/"))
		{
			...
			report.setReportBaseHeader(filePath+"/imagenes/logo-grey.gif", medico.getInstitucion(), medico.getNit(), tituloDiagnosticos);
		}
		else
		{
			filePath=filePath.substring(0, filePath.lastIndexOf("\\", (filePath.length()-2)));
		...
		report.setReportTitleSectionAtributes(0x000000, 0xFFFFFF,0xEBEBEB, 14);
