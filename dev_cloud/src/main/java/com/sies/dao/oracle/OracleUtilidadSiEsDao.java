 ...
 *
 */
public class OracleUtilidadSiEsDao implements UtilidadSiEsDao
{
	/**
	 * Manejador de losg de la clase
	 */
	private Logger logger=Logger.getLogger(OracleUtilidadSiEsDao.class);
	...
	public int obtenerValorSecuencia(Connection con, String nombreSecuencia)
	{
		try
		{
			PreparedStatement stm=con.prepareStatement("SELECT "+nombreSecuencia+".nextval AS valor_sec FROM DUAL");
		...
		{
			logger.error("Error ingresando el turno "+nombreSecuencia+": "+e);
		}
		return -1;
	}
