		...
		if(dto.isAdministracionPoblacionCapitada())
		{
			...
			if(stringMesesMaxVencimiento!=null&&!stringMesesMaxVencimiento.trim().isEmpty()){
				...
				if(mesesMaxVencimiento>0){
					criteria.add(Restrictions.sqlRestriction(sqlFechaVencimiento, new Object[]{nuevaFechaVen}, new Type[]{StandardBasicTypes.DATE}));
				}else{
					if(mesesMaxVencimiento==0){
						criteria.add(Restrictions.sqlRestriction(sqlFechaVencimiento, new Object[]{fechaActual}, new Type[]{StandardBasicTypes.DATE}));
					}
				}
			}
