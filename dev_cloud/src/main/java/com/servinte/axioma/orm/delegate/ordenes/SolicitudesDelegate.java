...
//			disjunctionFechas.add(Restrictions.between("anulacionSolicitudCx.fecha", fechaInicioAnulacion.getTime(), fechaFinAnulacion.getTime()));
	...
	*/
	public ArrayList<DtoPacientesPoliconsultadores> obtenerReportePoliconsultadores( 
		...
		if( !tipoIdentificacion.equals("") && !numeroIdentificacion.equals("") ){
			query.setParameter("tipoIdentificacion", tipoIdentificacion, StandardBasicTypes.STRING);
			query.setParameter("numeroIdentificacion", numeroIdentificacion, StandardBasicTypes.STRING);
		}else{
			query.setParameter("fechaInicial", fechaInicial, StandardBasicTypes.STRING);
			query.setParameter("fechaFinal", fechaFinal, StandardBasicTypes.STRING);
		}
		if( idConvenio != -1 ){
			query.setParameter("idConvenio", idConvenio, StandardBasicTypes.INTEGER);
		}
		if( idViaIngreso != -1 ){
			query.setParameter("idViaIngreso", idViaIngreso, StandardBasicTypes.INTEGER);
		}
		if( !tipoServicio.isEmpty() ){
			query.setParameter("tipoServicio", tipoServicio, StandardBasicTypes.STRING);
		}
		if( idEspecialidad != -1 ){
			query.setParameter("idEspecialidad", idEspecialidad, StandardBasicTypes.INTEGER);
		}
		if( idUnidadAgenda != -1 ){
			query.setParameter("idUnidadAgenda", idUnidadAgenda, StandardBasicTypes.INTEGER);
		}
		query.setParameter("esCapitado", ConstantesBD.acronimoSi, StandardBasicTypes.STRING);
