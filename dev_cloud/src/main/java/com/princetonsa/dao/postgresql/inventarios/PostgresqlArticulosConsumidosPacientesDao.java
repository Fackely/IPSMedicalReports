...
public class PostgresqlArticulosConsumidosPacientesDao implements ArticulosConsumidosPacientesDao 
{
	...
	private static String strConArticulosConsumidosPacientesParteUno = "SELECT " +
																			"to_char(dt.fecha_modifica, 'DD/MM/YYYY') || ' - ' || to_char(dt.hora_modifica, 'HH24:MM') AS fecha, " +
	...
	private static String strConArticulosConsumidosPacientesParteDos = "SELECT " +
																			"to_char(dp.fecha,'DD/MM/YYYY') || ' - ' || to_char(dp.hora, 'HH24:MM') AS fecha, " +
																			"getCodArticuloAxiomaInterfaz(va.codigo, '?') AS codigo, " +
																			"getdescarticulosincodigo(va.codigo) AS des, " +
																			"va.concentracion AS concen, " +
																			"va.forma_farmaceutica AS forma, " +
																			...
																			"getnumeroidentificaciontercero(e.tercero) AS id_ent, " +
																			"co.nombre AS entidad, " +
																			"coalesce(((ddp.cantidad::numeric) - coalesce((SELECT sum(ddpe.cantidad) FROM detalle_devol_pedido ddpe   INNER JOIN devolucion_pedidos dp ON(ddpe.devolucion=dp.codigo) WHERE dp.estado=2 AND ddpe.pedido=ddp.pedido and ddpe.articulo=ddp.articulo),0)),0) AS cantidad " +
																		"FROM " +
																			"pedido p " +
	 ...
	 */
   	public HashMap<String, Object> consultarCondicionesArticulosConsumidos(Connection con, HashMap<String, Object> criterios)
   	{
		...
		condicionesUno += "AND dt.fecha_modifica BETWEEN '"+UtilidadFecha.conversionFormatoFechaABD(criterios.get("fechaInicial")+"")+"' AND '"+UtilidadFecha.conversionFormatoFechaABD(criterios.get("fechaFinal")+"")+"' ";
		condicionesDos += "AND dp.fecha BETWEEN '"+UtilidadFecha.conversionFormatoFechaABD(criterios.get("fechaInicial")+"")+"' AND '"+UtilidadFecha.conversionFormatoFechaABD(criterios.get("fechaFinal")+"")+"' ";
		...
		mapa.put("consulta", consulta.replace('?', '0'));
