...
public class PostgresqlMovimientosAlmacenesDao implements MovimientosAlmacenesDao 
{
	/**
	 * 
	 */
	...
	public String obtenerSqlReporte(String centroAtencion, String almacen,
		...
		if(tipoReporte.equals(ConstantesIntegridadDominio.acronimoDetalladoTipoTransaccion))
			sql = "SELECT " +
					...
					"to_char(vmi.fechaatencion, 'DD/MM/YYYY') AS fechacierre, " +
					...
					"CASE WHEN vmi.cantidadentrada='0' THEN sum(coalesce(vmi.cantidadsalida)) ELSE sum(coalesce(vmi.cantidadentrada)) END AS cantidad, " +
