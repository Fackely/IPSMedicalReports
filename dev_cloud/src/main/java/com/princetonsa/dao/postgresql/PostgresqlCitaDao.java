...
* acceso a una base de datos PostgreSQL requeridos por la clase <code>Cita</code>.
*
* @version 1.0, Sep 22, 2003
* @author <a href="mailto:edgar@PrincetonSA.com">Edgar Prieto</a>
*/
public class PostgresqlCitaDao implements CitaDao
{

	/**
	 * Cadena que aumenta el valor de la secuencia de servicios cita
	 */
	private static final String secuenciaServiciosCita = "nextval('seq_servicios_cita')";
	
	/**
	* Cadena constante con el <i>Statement</i> necesario para aumentar
	* en 1 la secuencia de la cita en una BD de datos PostgreSQL
	*/
	private static final String is_aumentarSeqCita=
	"SELECT NEXTVAL('seq_cita')";
	...
	private static final String is_detalle =
		"SELECT "	+	"DISTINCT(ci.codigo)" + "AS codigoCita," +
						...
						"CASE "						+
							...
							"ELSE a.codigo "										+
						"END "						+ "AS codigoAgenda,"			+
						"TEXT(ci.fecha_gen)"		+ "AS fechaGeneracion,"			+
						"TEXT(ci.hora_gen)"			+ "AS horaGeneracion,"			+
						"ci.unidad_consulta "		+ "AS codigoUnidadConsulta,"	+
						"uc.descripcion "			+ "AS nombreUnidadConsulta,"	+
						"-1 "						+ "AS codigoServicio,"			+
						"coalesce(TEXT(a.fecha),'') AS fecha,"					+
						"coalesce(TEXT(a.hora_inicio),'') AS horaInicio,"				+
						"coalesce(TEXT(a.hora_fin),'') AS horaFin,"					+
	...
	*/
	...
	/**
	* Cadena constante con el <i>Statement</i> necesario para validar el numero de citras asiganas a
	* un paciente en una base de datos PostgreSQL en una fecha y hora de la agenda
	*/
	private static final String is_validarCitasFechaHora =
		"SELECT "	+	"COUNT(*) "				+
		"FROM "		+	"cita "					+ "c "		+
						"INNER JOIN agenda "	+ "a1 "		+ "ON(a1.codigo=c.codigo_agenda)"	+
						"INNER JOIN agenda "	+ "a2 "		+ "USING(fecha,hora_inicio)"					+
		"WHERE "	+	"c.codigo_paciente"		+	"=? AND "	+
						"a2.codigo"				+	"=? AND "	+
						"c.estado_cita IN("		+
							ConstantesBD.codigoEstadoCitaAsignada		+ ","	+
	...
	* reservada en una base de datos PostgreSQL
	*/
	...
	private static String asignarCitaCancelada="NEXTVAL('seq_cita')";
	...
	*/
	...
	{
		return SqlBaseCitaDao.listar(con, ai_modo, ai_paciente, ai_unidadConsulta, ai_consultorio, ai_medico, as_fechaSolicitada, as_fechaInicio, as_fechaFin, as_horaInicio, as_horaFin, ai_estadoCita, as_estadoLiquidacion, ai_cuenta, centroAtencion, centrosAtencion, unidadesAgenda);
	}
	...
	* existente en una BD Postgresql
	* 
	...
	* especificado en una BD Postgresql
	* 
	* @param ac_con				Conexiï¿½n abierta con una fuente de datos
	...
	* conexion existente en una BD Postgresql.
	* 
	* @param ac_con					Conexiï¿½n abierta con una fuente de datos
	...
	*/
	 ...
	 * necesaria para imprimirlas) para una BD Postgresql
	 * 
	* @see com.princetonsa.dao.CitaDao#listarCitas(String , boolean , String , boolean , String , boolean , String , boolean , int , boolean )
	...
	{
		campos.put("secuencia","nextval('seq_servicios_cita')");
