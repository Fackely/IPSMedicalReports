 ...
 *
 */
public class PostgresqlPacientesConEgresoPorFacturarDao implements PacientesConEgresoPorFacturarDao 
{
	...
	" case when c.via_ingreso = "+ConstantesBD.codigoViaIngresoHospitalizacion+" then to_char(ah.fecha_admision,'"+ConstantesBD.formatoFechaAp+"')||' '||to_char(ah.hora_admision,'hh24:mi') else to_char(au.fecha_admision,'dd/mm/yyyy')||' '||to_char(au.hora_admision,'hh24:mi') end as fechahoraadmision," +
	...
	" inner join egresos e on " +
	" (" +
				" e.cuenta=c.id " +
			"and " +
				"e.fecha_egreso is not null " +
			"and " +
				"e.hora_egreso is not null " +
			"and (" +
						"e.fecha_reversion_egreso is null " +
					"or " +
						"e.fecha_egreso>e.fecha_reversion_egreso or (e.fecha_egreso=e.fecha_reversion_egreso and e.hora_egreso>e.hora_reversion_egreso)) " +
