...
* servicios de acceso a una base de datos PostgreSQL requeridos por la clase
* <code>Solicitud</code>.
*
 * @version 2.0, Jan 14, 2003
 */

public class PostgresqlSolicitudDao implements SolicitudDao
{
	/**
	 * Maneja los logs del módulo de control de Solicitudes
	 */
	private static Logger logger = Logger.getLogger(PostgresqlSolicitudDao.class);
	
	/**
	 * obtiene el numero de solicitud a insertar para evitar problemas de consurrencia
	 */
	private static final String getNumeroSolicitudAInsertarStr="SELECT nextval('seq_real_solicitudes') as codigosolic";
	...
	// modificar
	// numero_autorizacion
	private static final String insertarSolicitudValoracionInicialStr="INSERT  into solicitudes (numero_solicitud, fecha_grabacion, hora_grabacion, fecha_solicitud, hora_solicitud, especialidad_solicitante, ocupacion_solicitada, centro_costo_solicitante, centro_costo_solicitado, consecutivo_ordenes_medicas, cuenta, cobrable, va_epicrisis, urgente, estado_historia_clinica, tipo ) values (?, CURRENT_DATE, "+ValoresPorDefecto.getSentenciaHoraActualBD()+", CURRENT_DATE, "+ValoresPorDefecto.getSentenciaHoraActualBD()+", 0, 0, ?, ?, nextval('seq_sol_consecutivo_base'), ?, ?, true, true, 1, ? )";
	...
	private static final String insertarSolicitudGeneralInternaStr ="INSERT INTO solicitudes (numero_solicitud, consecutivo_ordenes_medicas, " +
			...
			"VALUES (?, nextval('seq_sol_consecutivo_base'), CURRENT_DATE, "+ValoresPorDefecto.getSentenciaHoraActualBD()+
			", ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?||' '||?||' '||getnombremedico(?)||' R.M. '||" +
	...
	// modificar
	// numero_autorizacion, 
	private static final String insertarSolicitudGeneralExternaStr="INSERT INTO solicitudes (numero_solicitud, consecutivo_ordenes_medicas, fecha_grabacion, hora_grabacion, fecha_solicitud, hora_solicitud, tipo, cobrable, especialidad_solicitante, ocupacion_solicitada, centro_costo_solicitante, centro_costo_solicitado, codigo_medico, cuenta, va_epicrisis, urgente, estado_historia_clinica, datos_medico,pyp, tiene_cita, liq_asocios,justificacion_solicitud, especialidad_solicitada) VALUES (?, nextval('seq_sol_consecutivo_otros'), CURRENT_DATE, "+ValoresPorDefecto.getSentenciaHoraActualBD()+", ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	...
	private static final String insertarTratanteStr="INSERT INTO tratantes_cuenta " +
		...
		"values " +
		"(nextval('seq_tratantes_cuenta'), ?, ?, CURRENT_DATE, null, "+ValoresPorDefecto.getSentenciaHoraActualBD()+", null, ?,(SELECT cuenta FROM solicitudes WHERE numero_solicitud = ?))";
	 ...
	 */
	private static final String insertarEntradaSolicitudesCambioTratanteStr="INSERT INTO sol_cambio_tratante (codigo, cuenta, solicitud, activa) values (nextval('SEQ_SOL_CAMBIO_TRAT'), ?, ?, true)";
	...
	private static final String insertarAdjuntoCuentaStr="INSERT INTO adjuntos_cuenta " +
		...
		"values " +
		"(nextval('seq_adjuntos_cuenta'), ?, ?, CURRENT_DATE, CURRENT_TIME, ?, (SELECT cuenta FROM solicitudes WHERE numero_solicitud = ? ))";
	...
	private static final String insertarAdjuntoAnteriorStr="INSERT into adjuntos_cuenta " +
			"(codigo, solicitud, codigo_medico, fecha_inicio, hora_inicio, centro_costo, cuenta) " +
			"values " +
			"(nextval('seq_adjuntos_cuenta'), ?, ?, ?, ?, ?, (SELECT cuenta FROM solicitudes WHERE numero_solicitud = ?))";
...
/*
	 ...
	 */
//	private static final String buscarSolicitudesActivasStr="SELECT ct.solicitud as numeroSolicitud, sol.codigo_medico as codigoMedico, sol.centro_costo_solicitado as codigoCentroCosto from sol_cambio_tratante ct, solicitudes sol where ct.cuenta=? and activa=true and ct.solicitud=sol.numero_solicitud";
...
/*
	 ...
	 */
//	private static final String insertarCancelacionTratanteStr="INSERT INTO cancelacion_tratantes (codigo, codigo_medico, fecha, hora) values ((SELECT max(codigo) from sol_cambio_tratante where cuenta =?), ?, CURRENT_DATE, CURRENT_TIME)";
...
//	private static final String cancelarSolicitudCambioManejoAnteriorStr="UPDATE sol_cambio_tratante set activa=false where cuenta = ?";
	...
	public int insertarSolicitudSubCuenta(Connection con, DtoSolicitudesSubCuenta dto) throws BDException
	{
		...
		try	{
			...
			String cadena="select nextval('seq_sol_sub_cuentas') ";
