 ...
 */
public class OracleEstanciaAutomaticaDao implements EstanciaAutomaticaDao {
	/**
	...
	private static final String consultaTiposMonitoreoSinDiaEstanciaStr="SELECT "+ 
		...
		"WHERE om.cuenta=? AND eh.fecha_orden<? AND rownum=1 " +
		"ORDER BY eh.fecha_orden DESC, eh.hora_orden DESC";
	...
	private static final String insertarLogEstanciaAutomaticaStr="INSERT INTO estancia_automatica " +
		...
		"VALUES (seq_estancia_automatica.nextval,?,?,?,CURRENT_DATE,"+ValoresPorDefecto.getSentenciaHoraActualBD()+",?,?,?,?,?,?,?,?)";
	...
	private static final String consultaLogEstanciaAutomaticaStr="SELECT "+ 
		...
		"to_char(e.fecha_grabacion,'DD/MM/YYYY') ||' '|| e.hora_grabacion AS fecha_grabacion,"+
		...
		"WHERE "+
		"e.fecha_inicial_estancia between to_date(?,'YYYY-MM-DD') AND to_date(?,'YYYY-MM-DD')";
	/**
	...
	public HashMap consultarTrasladosCuenta(Connection con,int idCuenta,String fechaEstancia,int centroCosto)
	{
		...
		consulta+=" ORDER BY tc.fecha_asignacion DESC,tc.hora_asignacion DESC";
