 ...
 */
public class OracleListadoCitasDao implements ListadoCitasDao
{
	private static final String consultaCitasPorPaciente=
		...
		"coalesce(  ( " +
			...
			"WHERE " +
				"sc2.codigo_cita = c.codigo and scu2.nro_prioridad=1 and rownum=1 " +
			"), " +
		...
		" FROM agenda a INNER JOIN cita c " +
		" ON(c.codigo_agenda=a.codigo) " +
		" INNER JOIN consultorios co " +
		" ON(co.codigo=a.consultorio) " +
		" INNER JOIN personas pp " +
		" ON(pp.codigo=c.codigo_paciente) " +
		" INNER JOIN unidades_consulta uc " +
		" ON(uc.codigo=c.unidad_consulta) " +
		" LEFT OUTER JOIN personas pm " +
		" ON(pm.codigo=a.codigo_medico) " +
		" LEFT OUTER JOIN contratos cont " +
		" ON (cont.codigo=c.contrato) " +
		" LEFT OUTER JOIN convenios conv " +
		" ON (conv.codigo=c.convenio) " +
		" LEFT OUTER JOIN servicios_cita cc " +
		" ON (cc.codigo_cita =c.codigo " +
		" AND cc.control_post_operatorio_cx  IS NULL) " +
		" INNER JOIN estados_liquidacion el    " +
		" ON(el.acronimo=c.estado_liquidacion) " +
		" INNER JOIN estados_cita ec    " +
		" ON(ec.codigo=c.estado_cita) " +
		" LEFT OUTER JOIN estratos_sociales estra  " +
		" ON (estra.codigo=c.estrato_social) " +
		" LEFT OUTER JOIN tipos_afiliado tipa  " +
		" ON (tipa.acronimo=c.tipo_afiliado) ";
	...
	{
		return SqlBaseListadoCitasDao.obtenerFechaPrimeraCitaPaciente(con, codigoPaciente, DaoFactory.ORACLE);
	}
}
