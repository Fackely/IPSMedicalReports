 ...
 */
public class OracleUtilidadesManejoPacienteDao implements UtilidadesManejoPacienteDao 
{
	
	private static Logger logger=Logger.getLogger(OracleUtilidadesManejoPacienteDao.class);
	...
	{
		return SqlBaseUtilidadesManejoPacienteDao.obtenerEstudios(con,DaoFactory.ORACLE);
	}
	...
	public ArrayList<DtoCentrosAtencion> obtenerCentrosAtencion(DtoCentrosAtencion dto) 
	{
		...
		String consultaStr= " Select 	consecutivo  as consecutivo ," +
			...
			consultaStr+=" AND  activo="+(dto.isActivo()?"1":"0"); 
	...
	{
		campos.put("secuencia", "seq_log_estados_cama.nextval");
	...
	{
		return SqlBaseUtilidadesManejoPacienteDao.actualizarEstadoPreingreso(con, ingreso, estado, usuario,DaoFactory.ORACLE);
	}
	...
	public DtoCertAtenMedicaFurips cargarCetificacionMedicaFurips(int ingreso) {
		return SqlBaseUtilidadesManejoPacienteDao.cargarCetificacionMedicaFurips(ingreso,DaoFactory.ORACLE);
	}
