 ...
 */
public class OracleProgramacionCirugiaDao implements ProgramacionCirugiaDao {
	
	
	/**
	 * Metodo para retornar la información de encabezado de peticion de cirugia 
	 * @param con
	 * @param nroPeticion
	 * @param tipoConsulta
	 * @return
													  ...
													  String fechaFinCirugia, int profesionalSolicita, int centroAtencion)
	{
		...
		consulta.append(
				  ...
				  "CASE WHEN psqx.hora_inicio IS NULL THEN '' ELSE psqx.hora_inicio END AS hora_cirugia, "+
	...
	public Collection cargarProgramacionSalas(Connection con, String fechaProgramacion)
	{
		...
		consulta.append("  SELECT * from (SELECT peticion as peticion, sala as sala, hora_inicio as hora_inicio," +
						"						 hora_fin as hora_fin "+  
						"						 FROM programacion_salas_qx pqx " +
						" 							  WHERE to_char(fecha_cirugia, 'YYYY-MM-DD') = ? " +
						"  UNION ALL  " +
						"  SELECT -1 as peticion, codigo_sala as sala, rango_inicial as hora_inicio, rango_final as hora_fin " +
