 ...
 *
 */
public class OracleFormatoJustArtNoposDao implements FormatoJustArtNoposDao {
	...
	public HashMap cargarInfoIngresoRango(Connection con, HashMap ingresosMap, HashMap filtro, String articulos,String fechaini, String fechafin, int tipocodigo){
		...
		String where = " FROM "
			...
			+ " AND to_date(s.fecha_solicitud)  between  to_date('"+fechaini+"','DD/MM/YY')  AND to_date('"+fechafin+"','DD/MM/YY')"
	...
	String 	where = " where " 
		...
		+ " AND to_date(jas.fecha)  between  to_date('"+fechaini+"','DD/MM/YY')  AND to_date('"+fechafin+"','DD/MM/YY')"
	...
	if (filtro.get("tipoRompimiento").toString().equals("centro_costo_"))
		group += " s.centro_costo_solicitante,";
	group += "  getdescarticulosincodigo(jas.articulo)  ," + " jas.articulo,";
	if (filtro.get("tipoRompimiento").toString().equals("piso_"))
		group += " getnombrepiso(getcodigopisocuenta(c.id,c.via_ingreso)),";
	if (filtro.get("tipoRompimiento").toString().equals("estadojus_"))
		group += " getintegridaddominio(jar.estado),";

	group += "  getnombrepersona(jaf.profesional_responsable), art.categoria " + "ORDER BY  codigomed, articulo asc";
