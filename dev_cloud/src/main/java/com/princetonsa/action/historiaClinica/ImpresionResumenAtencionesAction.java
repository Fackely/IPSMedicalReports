...
//				dto.setResultadoSeccionEvolucion(ImpresionResumenAtenciones.consultarInformacionHistoriaClinica(con,filtro,ConstantesImpresionHistoriaClinica.codigoSeccionEvoluciones));
			...
			if(!varios){
				...
				JasperReportBuilder reporte2 = generarPdf(dto,filtro,mundo.obtenerEncabezadoPaciente(con, forma.getIdIngreso()),usuario,paciente,institucionActual,forma.getIdIngreso(),imprimirSoloNotasAclaratorias,primerIngreso,/*varios*/true);
				reporte.summary(cmp.subreport(reporte2));		
			}else{

				//EN CASO DE IMPRIMIR VARIOS INGRESOS
				JasperReportBuilder reporte2=generarPdf(dto,filtro,mundo.obtenerEncabezadoPaciente(con, forma.getIdIngreso()),usuario,paciente,institucionActual,forma.getIdIngreso(),imprimirSoloNotasAclaratorias,primerIngreso,varios);
				...
				if(primerIngreso){
					...
					JasperReportBuilder reporte3=new JasperReportBuilder();
					reporte3.summary(cmp.subreport(reporte2));
					reporte.summary(cmp.subreport(reporte3.summaryOnANewPage()));
				}
			}
		}

		//SI SE IMPRIMEN VARIOS INGRESOS SE CONSOLIDA EN UNO REPORTE QUE CONTIENE MAS REPORTES Y SE ADICIONA FOOTER PARA LA UNICA NUMERACION 
		//hermorhu - MT6422 
//		if(varios){
