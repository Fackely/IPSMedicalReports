/******************************************************************************
 * Encoder.java - encode characters to HTML entities.
 * Copyright (C) 2002 Matthew M Lavy
 * mml@mupsych.org
 *
 ...
 * In addition, it defines a Writer that filters a text stream, replacing
 * characters with their entity equivalents. <p>
 * It is intended for use by programmers writing web applications, who need
 * to ensure (a) that text sent to browsers does not contain characters such
 * as &lt; (which can break the HTML), and that (b) characters such as &copy;
 * display properly in the browser.
 * <p>
 * The Encoder has two use patterns:
 * <ul>
 * <li>Call one of 'encode()', 'encodeNumeric()', 'getEntity()' or
 * 'getNumericEntity()' to convert strings or single characters.
 * <li>Use the Writer returned by 'createWriter()' to convert a text stream
 * on the fly.
 * </ul>
 * Both of these patterns are demonstrated in EncoderTest.java.
 * <p>
 * The Encoder understands all of the character entities referenced
 * in http://www.w3.org/TR/REC-html40/sgml/entities.html. This includes:
 * <ul>
 * <li>Character entities for ISO 8859-1 characters.
 * <li>Character entities for greek letters and symbols (mathematical and
 * otherwise).
 * <li>Character entities for "markup-significant" and internationalization
 * characters.
 * </ul>
 */
...
public class Encoder
{
	...
	public static String encode(String s)
	{
		...
		for (int i=0; i<chars.length; i++) {
			sb.append(Encoder.getEntity(chars[i]));
		}
		return sb.toString();
	}
	...
	public static String encodeNumeric(String s)
	{
		...
		char[] chars = s.toCharArray();
		for (int i=0; i<chars.length; i++) {
			sb.append(Encoder.getNumericEntity(chars[i]));
		}
		return sb.toString();
	}
		...
		return new FilterWriter(out) {
			...
			throws IOException {
				for (int i=off; i<off+len; i++) {
					this.out.write(Encoder.getEntity(cbuf[i]));
				}
			}
			...
			public void write(int c) throws IOException {
			   this.out.write(Encoder.getEntity((char)c));
			}
