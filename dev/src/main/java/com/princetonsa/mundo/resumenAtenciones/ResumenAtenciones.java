...
public class ResumenAtenciones
{
			...
			String fechaIngresoFinal, String centroAtencion,String viaIngreso,String especialidad)
 {
	 ...
	 * numeroObservacionesSolicitudesIgualesValUrgHosp - Método que determina si las observaciones 
	 * 	de determinadas solicitudes (Urgencias / Hospitalización) son iguales o no.
	 * @param con
	 * @param ingreso
	 * @return true: las observaciones son iguales - false: las observaciones son diferentes.
	 */
	public boolean numeroObservacionesSolicitudesIgualesValUrgHosp (String ingreso) throws SQLException {
		boolean returnValue = false;
		Valoraciones valoracionMundo = new Valoraciones();
		Connection con=UtilidadBD.abrirConexion();
		try {
			//Se consultan los números de solicitud de urgencias y hospitalización que tiene el ingreso.
			List<String> numerosSolicitud = resumentAtencionesDao.consultarNumeroSolicitudXIngreso(con, ingreso);
			if (!numerosSolicitud.isEmpty() && numerosSolicitud.size()>1) {
				//Se obtienen las observaciones de cada solicitud (Urgencias/Hospitalización)
				List<DtoValoracion> solicitudUrgencias = valoracionMundo.obtenerValoracionesObservacion(con, null, null, numerosSolicitud.get(0));
				List<DtoValoracion> solicitudHospitalizacion = valoracionMundo.obtenerValoracionesObservacion(con, null, null, numerosSolicitud.get(1));
				if (solicitudUrgencias.size() == solicitudHospitalizacion.size()) {
					returnValue = true;
				}
			} else if(numerosSolicitud.size()==1){
				returnValue = true;
			}
		} catch (Exception ex) {
			ex.printStackTrace();
		...
		//Se retorna el valor dependiendo si las observaciones son iguales o no.
		return returnValue;
	}
}