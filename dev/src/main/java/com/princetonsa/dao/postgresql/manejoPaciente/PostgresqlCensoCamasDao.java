...
public class PostgresqlCensoCamasDao implements CensoCamasDao
{
	...
	public HashMap consultaCenso (Connection connection,HashMap parametros)
	{
		...
		return SqlBaseCensoCamasDao.consultaCenso(connection, parametros, sqlCensoPostgres);
	}
	...
	{
		String srtCadenaConsultaFechaHoraOcupacionReserva = " SELECT " +
		" to_char(lrc.fecha,'DD/MM/YYYY')  ||'@@@@@'|| lrc.hora " +
		" FROM log_reservar_cama lrc " +
		" WHERE lrc.codigo_cama=? ORDER BY lrc.codigo DESC "+ValoresPorDefecto.getValorLimit1()+" 1";
	...
	public String ConsultaFechaOcupacionOtrosEst(Connection connection, int cuenta)
	{
		String sentenciaSQL = "SELECT "
			...
			+ "FROM log_estados_cama lec "
			+ "WHERE lec.cuenta=? ORDER BY lec.codigo DESC "
	...
	public boolean corrrerProcesoAutomatico ()
	{
		...
		" h.piso, h.centro_atencion, c.institucion, nextval('seq_proc_aut_censo')" +
		" FROM camas1 c " +
