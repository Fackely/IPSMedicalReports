...
*/
public class PostgresqlSolicitudesDao implements SolicitudesDao
{
	/**
	 * 
	 */
	...
	private static final String is_listar =
				"SELECT so.numero_solicitud AS numeroSolicitud," +
					...
					" case when so.pyp is null or so.pyp is false then 'false' else 'true' end as pyp, " +
					" TEXT(so.fecha_solicitud) AS fechaSolicitud," +
					" TEXT(so.hora_solicitud) AS horaSolicitud," +
					" TEXT(so.fecha_grabacion) AS fechaGrabacion," +
					" TEXT(so.hora_grabacion) AS horaGrabacion," +
					" CASE WHEN so.fecha_interpretacion IS NULL THEN '' ELSE TEXT(so.fecha_interpretacion) END AS fechaInterpretacion, " +
					" CASE WHEN so.hora_interpretacion IS NULL THEN '' ELSE TEXT(so.hora_interpretacion) END AS horaInterpretacion," +
					" '' AS fechaRespuesta," +
					" '' AS horaRespuesta," +
					...
					" gettieneportatilsolicitud(so.numero_solicitud) As portatil," +
					...
					" case when so.tipo = "+ConstantesBD.codigoTipoSolicitudProcedimiento+" then getSolProcMultiple(so.numero_solicitud)  else false end as multiple, " +
					...
					" getSolicitudTieneIncluidos(so.numero_solicitud)>0 as incluyeServiciosArticulos "+
		" FROM solicitudes so " +  
	...
	private static final String insertarLogRevisionCuentaStr = "INSERT " +
		...
		"VALUES " +
		"(nextval('seq_tarifas_rev_cuenta'),?,?,?,?,?,?,?,?,CURRENT_DATE,CURRENT_TIME,?)";
	...
	*/
	...
	)throws SQLException
	{
		...
		*/
		...
		return
			UtilidadBD.resultSet2Collection(
					new ResultSetDecorator(new PreparedStatementDecorator(ac_con.prepareStatement(lsb_sb.toString(),ConstantesBD.typeResultSet,ConstantesBD.concurrencyResultSet) ).executeQuery())
			);
	}
