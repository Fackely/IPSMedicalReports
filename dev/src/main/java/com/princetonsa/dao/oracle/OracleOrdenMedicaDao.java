 ...
 */
public class OracleOrdenMedicaDao implements OrdenMedicaDao
{
	String cadenaHoraOrden="enc.hora_orden";
	String cadenaHoraGrabacion="enc.hora_grabacion";
	
	/**
	 *   
	 */
	private String strConsultaListadoNitOral = "SELECT * FROM (SELECT od.codigo_histo_enca AS codigo, od.via_oral as via_oral, od.finalizar_dieta as finalizar_dieta, tno.nombre as nutricion_oral, " +
	...
	{
		String secuencia="seq_ordenes_medicas.nextval as codigo FROM DUAL";
	...
	public  DtoResultado insertarOrdenMedica(Connection con,  int cuenta, String descripcionSoporteRespiratorio, String descripcionDieta,  String observacionesGenerales, String descripcionDietaParenteral,Boolean tieneDatos)
	{
		...
		String consultaCodigoDetalleObservacion = "select ordenes.seq_detalle_obs_orden_medic.nextval from dual";
		
		
		return SqlBaseOrdenMedicaDao.insertarOrdenMedica(con,  secuencia, cuenta, descripcionSoporteRespiratorio, descripcionDieta,  observacionesGenerales, descripcionDietaParenteral,consultaCodigoDetalleObservacion,tieneDatos);
	}
	...
	/**
	 * Funcion para insertar los tipos de nutriciones
	 * @param numero de la orden medica
	 * @param tipo de nutricion especifica ordenada   
	 * @throws SQLException
	 * @throws SQLException
	 */
	...
	{
		return SqlBaseOrdenMedicaDao.consultarTipos(con, institucion, centro_costo, Nro_consulta, mezcla, codigoCuenta, cuentaAsocio, inicioEncabezado, finEncabezado, DaoFactory.ORACLE);
	}
	...
	{
		String codigoCuidadoEnf = "seq_orden_cuidado_enfermeria.nextval as codigo";
	...
	public Collection consultarCuidadosEnfHisto(Connection con, int codigoCuenta, int cuentaAsocio, int institucion, int centroCosto,String fechaInicial,String fechaFinal)
	{
		...
		consultaBuffer.append( " SELECT * FROM (" +
							   					" (SELECT   1 as medico," +
							   						" to_char (enc.fecha_grabacion,'DD/MM/YYYY')  || '  --  ' || enc.hora_grabacion as fecha_hora_grab, " +
							   						" to_char (enc.fecha_orden,'DD/MM/YYYY')  || '  --  ' || enc.hora_orden as fecha_hora," +
		...
		consultaBuffer.append(" AND to_char(enc.fecha_orden, 'YYYY-MM-DD') between '"+fechaInicial+"' and '"+fechaFinal+"'");
		...
		consultaBuffer.append(" AND to_char(enc.fecha_orden, 'YYYY-MM-DD') between '"+fechaInicial+"' and '"+fechaFinal+"'");
			...
			consultaBuffer.append(" SELECT 0 as medico, " +
								  		" to_char (ehre.fecha_grabacion,'DD/MM/YYYY')  || '  -- ' || ehre.hora_grabacion as fecha_hora_grab, " +  
								  		" to_char (ehre.fecha_registro,'DD/MM/YYYY')  || '  --  ' || ehre.hora_registro as fecha_hora, " +
								  ...
								  " INNER JOIN tipo_cuidado_enfermeria tce ON ( ceci.cuidado_enfermeria = tce.codigo ) " +
								  "	" + cue + " " +
								  " AND to_char(ehre.fecha_registro, 'YYYY-MM-DD') between '"+fechaInicial+"' and '"+fechaFinal+"'" +
								  "	UNION " +
								  "	SELECT 0 as medico, " +
								  		" to_char (ehre.fecha_grabacion,'DD/MM/YYYY')  || '  --  ' || ehre.hora_grabacion as fecha_hora_grab, " +	 
								  		" to_char (ehre.fecha_registro,'DD/MM/YYYY')  || '  --  ' || ehre.hora_registro as fecha_hora, " + 
   ...
   * @return Collection con los datos del tipo de monitoreo
	...
	{
		String secuencia="seq_otro_nutricion_oral.nextval";
		
		if(System.getProperty("TIPOBD").equals("ORACLE"))
			secuencia += " as codigo FROM DUAL";
