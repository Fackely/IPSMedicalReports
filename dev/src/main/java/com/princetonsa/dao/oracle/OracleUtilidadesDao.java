 ...
 * Princeton S.A.
 */
public class OracleUtilidadesDao implements UtilidadesDao
{
	/**
	 * Manejador de logs de la clase
	 */
	private Logger logger=Logger.getLogger(OracleUtilidadesDao.class);
	
	/**
	 * Cadena usada para consultar la descripciï¿½n de un esquema tarifario
	 * general o especï¿½fico
	 */
	private static final String obtenerNomEsquemaTarifarioStr="select getnomesqporcentaje(?,?) As esquema_tarifario FROM dual";
	...
	private static String strIsertarIngresoCuidadosEspeciales="INSERT INTO ingresos_cuidados_especiales (codigo," +
																			...
																			"centro_costo" +
																			") " +
																"VALUES (historiaclinica.seq_ingres_cuidados_especial.nextval," +
																			"?," +
																			"?," +
																			"?," +
																			"?," +
	...
	{
		String consultaFechaStr="SELECT to_char(current_date, 'YYYY-MM-DD') AS fecha FROM DUAL";
	...
	{
		String consultaHoraStr="SELECT to_char(current_timestamp,'HH24:MI') AS hora from dual";
	...
	{
		String cadena="select codigo as codigo from (select * from tipos_cie where vigencia<=current_date order by vigencia desc) where rownum=1";
	...
	{
		return SqlBaseUtilidadesDao.obtenerIdUltimaCuenta(con,codigoPaciente, DaoFactory.ORACLE);
	}
	...
	{
		String sentencia="SELECT getUltimoValorAlturaUterina("+institucion+","+codigoPaciente+") AS altura_uterina FROM dual";
	...
	{
		String cadena="select "+nombreSecuencia+".NEXTVAL as secuencia FROM dual"; 
	...
	{
		String busquedaPoolesCuenta="SELECT pp.pool AS pool, pool.descripcion AS descripcion FROM participaciones_pooles pp INNER JOIN pooles pool ON(pool.activo=1 AND pool.codigo=pp.pool) WHERE medico=(SELECT distinct(CASE WHEN sol.codigo_medico_responde IS NULL THEN (SELECT distinct(a.codigo_medico) FROM cita c INNER JOIN agenda a ON(a.codigo=c.codigo_agenda) WHERE numero_solicitud=sol.numero_solicitud) ELSE sol.codigo_medico_responde END) from solicitudes sol where sol.cuenta=? and sol.tipo NOT IN ("+ConstantesBD.codigoTipoSolicitudMedicamentos+","+ConstantesBD.codigoTipoSolicitudCargosDirectosArticulos+") and sol.centro_costo_solicitado<>"+ConstantesBD.codigoCentroCostoExternos+") AND (pp.fecha_ingreso<CURRENT_DATE or (pp.fecha_ingreso=CURRENT_DATE and pp.hora_ingreso<="+ValoresPorDefecto.getSentenciaHoraActualBD()+")) and (pp.fecha_retiro IS NULL or (pp.fecha_retiro>CURRENT_DATE OR (pp.fecha_retiro=CURRENT_DATE AND pp.hora_retiro>=CURRENT_TIME)))";
		try
		{
    ...
    {
    	String orderBy = " AND rownum =1 ORDER BY codigo DESC ";
	...
	{
		String consultaHoraSegundosStr="SELECT to_char(current_timestamp ,'HH24:MI:SS') AS hora FROM DUAL";
	...
	{
		return SqlBaseUtilidadesDao.getTotalAdminFarmacia(con, codigoArticuloStr, numeroSolicitudStr, traidoUsuario, " from dual ");
	}
	...
	{
		/**MT 4452 Diana Ruiz*/
		//String adicion = " AND rownum = 1 ORDER BY codigo DESC";
		String consulta = "select cama from (SELECT codigo_nueva_cama AS cama FROM traslado_cama WHERE cuenta = ? ORDER BY fecha_grabacion desc,hora_grabacion desc) where rownum=1";
	...
	public boolean esNivelServicioContratado(Connection con, String servicio, int contrato)
	{
		...
		try {
			ps= new PreparedStatementDecorator(con.prepareStatement("select getesnivelservcontratado("+contrato+","+servicio+") as resultado from dual",ConstantesBD.typeResultSet,ConstantesBD.concurrencyResultSet));
	...
	{
		String secuencia = "seq_acum_act_prog_pac_pyp.nextval";
	...
	public String obtenerNombreUsuarioXLogin(Connection con, String login, boolean primeroApellidos)
	{
		...
		if(primeroApellidos)
		   consultaStr="SELECT getnombreusuario2(?) AS nombre_usuario FROM dual";
	      else
		   consultaStr="SELECT getnombreusuario(?) AS nombre_usuario FROM dual";
	...
	{
		return SqlBaseUtilidadesDao.obtenerConvenios(con,campos, DaoFactory.ORACLE);
	}
	...
	{
		return SqlBaseUtilidadesDao.obtenerConveniosContrato(con,campos, DaoFactory.ORACLE);
	}
	...
	/**
	 * Mï¿½todo que consulta los conceptos glosas por institucion
	 * @param con
	 * @param institucion
	 * @return
	 */
	...
	{
		return SqlBaseUtilidadesDao.obtenerContratos(con,codigoConvenio,todos, validarNumeroContratos,pendientesFinalizar,DaoFactory.ORACLE);
	}
	...
	public Vector<InfoDatosString> obtenerNaturalezasPacienteXTipoAfiliadoEstrato(
		...
		return SqlBaseUtilidadesDao.obtenerNaturalezasPacienteXTipoAfiliadoEstrato(con, tipoRegimen, convenio, viaIngreso, codigoTipoAfiliado, codigoEstratoSocial, DaoFactory.ORACLE, fechaReferencia);
	}
	...
	{
		String cadena="INSERT INTO movimientos_abonos(codigo,paciente,codigo_documento,tipo,valor,fecha,hora,institucion) values(seq_movimientos_abonos.nextval,?,?,?,?,?,?,?)";
	...
	{
		String consulta="SELECT manejoPaciente.getabonodisponible(?,?) AS abono_disponible FROM DUAL";
		try
		{
	...
	{
		return SqlBaseUtilidadesDao.consultaUltimoEgresoEvolucion(con, codigoPaciente, viaIngreso, DaoFactory.ORACLE);
	}
	...
	{
		return SqlBaseUtilidadesDao.marcarReingreso(con, codIngreso, codPaciente, DaoFactory.ORACLE);
	}
	...
	/**
	 * Metodo inserta un registro en Ingresos por Cuidados Especiales
	 * @param con
	 * @param ingreso
	 * @param estado
	 * @param indicativo
	...
	{
		String secuenciaStr=" historiaclinica.seq_ingres_cuidados_especial.nextval ";
		return SqlBaseUtilidadesDao.insertarIngresoCuidadosEspeciales(con, ingreso, estado, indicativo, tipoMonitoreo, usuario, evolucion, valoracion,centroCosto, strIsertarIngresoCuidadosEspeciales, secuenciaStr);
	}
	
	/**
	 * Metodo para actualizar el estado a Finalizado de un Ingreso de Cuidados Especiales
	 * @param con
	 * @param ingreso
	 * @return
	 */
	...
	/**
	 * Metodo de consulta para los totales de un factura y su impresion
	 * @param con
	 * @param codFactura
	 * @return
	 */
 ...
 * @return */
	 ...
	 * 
	 * @param con
	 * @param codProveedores
	 * @return
	 */
	public HashMap obtenerProveedores(Connection con, int codProveedores)
	{
		return SqlBaseUtilidadesDao.obtenerProveedores(con, codProveedores);
	}
	...
	public boolean actualizarFormatoFechaBD() 
	{
		...
		try
		{
			PreparedStatementDecorator ps= new PreparedStatementDecorator( new PreparedStatementDecorator(con.prepareStatement(sql, ConstantesBD.typeResultSet,ConstantesBD.concurrencyResultSet)));
			retorna= ps.executeUpdate()>0;
			ps.close();
		}
		catch (SQLException e) 
		{
			logger.error("NO ACTUALIZO  EL FORMATO DE FECHA");
		}
	...
	public boolean betweenFechas(String fechaEvaluar, String horaEvaluar,
		...
		String consulta="select 1 from dual where " +
			/*
				"	('"+fechaEvaluar+"'='"+fechaInicial+"' and '"+horaEvaluar+"'>='"+horaInicial+"') or " +
				" 	('"+fechaEvaluar+"'>'"+fechaInicial+"' and '"+fechaEvaluar+"'<'"+horaEvaluar+"') or " +
				" 	('"+fechaEvaluar+"'='"+fechaFinal+"'   and '"+horaEvaluar+"' <= '"+horaFinal+"')";
			*/
				"	('"+fechaEvaluar+"'='"+fechaInicial+"' and '"+horaEvaluar+" '>='"+horaInicial+"') or " +
				" 	('"+fechaEvaluar+"'>'"+fechaInicial+"' and '"+horaInicial+"'<'" +horaEvaluar+"') or " +
				" 	('"+fechaEvaluar+"'='"+fechaFinal+"'   and '"+horaEvaluar+" '<='"+horaFinal+"')";
	...
	@Override
	public ArrayList<HashMap<String, Object>> consultarMotivosCita(
			Connection con, int codInstitucion,String tipoMotivo) {
		return SqlBaseUtilidadesDao.consultarMotivosCita(con, codInstitucion,tipoMotivo);
	}

	@Override
	public ArrayList<HashMap<String, Object>> consultarMediosConocimientoServ(
			Connection con, int codInstitucion) {
	...
	{
		return SqlBaseUtilidadesDao.getDiagnosticoPacienteIngreso(con,idIngreso,DaoFactory.ORACLE);
	}
	
	@Override
	public DtoDiagnostico getDiagnosticoPacienteCuenta(Connection con, int idCuenta) 
	{
		return SqlBaseUtilidadesDao.getDiagnosticoPacienteCuenta(con,idCuenta,DaoFactory.ORACLE);
	}
	...
	/**
	 * @see com.princetonsa.dao.UtilidadesDao#obtenerNombreViaAdministracion(java.sql.Connection, int)
	 */
