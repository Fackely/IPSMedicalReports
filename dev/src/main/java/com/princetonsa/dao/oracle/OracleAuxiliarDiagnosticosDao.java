/*
 * @(#)OracleAuxiliarDiagnosticosDao.java
 *
 ...
 * de acceso a una base de datos Oracle requeridos por la clase <code>AuxiliarDiagnosticos</code>
 *
 * @version 1.0, Jun 16, 2003
 */
public class OracleAuxiliarDiagnosticosDao implements AuxiliarDiagnosticosDao
{
	/**
	 * Objeto para manejar los problemas que se presenten en esta
	 * clase
	 */
	private Logger logger = Logger.getLogger(OracleAuxiliarDiagnosticosDao.class);
	 ...
	 * si no retorna -1
	 */
	private static final String obtenerFechaYHoraUltimaEvolucionStr="SELECT evol.codigo as codigoUltimaEvolucion, to_char(evol.fecha_evolucion, 'YYYY-MM-DD') as fechaEvolucion, evol.hora_evolucion as horaEvolucion from evoluciones evol, solicitudes sol where  sol.cuenta=? and sol.numero_solicitud=evol.valoracion order by evol.fecha_evolucion desc, evol.hora_evolucion desc";
	...
	"SELECT evold.acronimo_diagnostico as acronimoDiagnostico, diag.nombre as diagnostico, evold.tipo_cie_diagnostico as tipoCieDiagnostico, evold.principal, evold.definitivo, 1 as numero from evol_diagnosticos evold, diagnosticos diag where evold.acronimo_diagnostico=diag.acronimo and evold.tipo_cie_diagnostico=diag.tipo_cie and evold.evolucion=?";
	 ...
	 * 
	 * @param con Conexión con BD Oracle
	 ...
	 * 
	 * @param con Conexión con la BD Oracle
	...
	private ResultSetDecorator cargarDiagnosticosEvolucion (Connection con, int numeroSolicitud, int idCuenta, boolean esEvolucion) throws SQLException
	{
		try
		{
			...
			if (esEvolucion)
			{
				// en oracle es un camello ponerle este orden
				String consultaReal="SELECT * FROM ( (" + cargarDiagnosticosEvolucion2Str + ") UNION (" + cargarDiagnosticosEvolucionExtraStr + ") ) tabla "/*ORDER BY tabla.fecha_valoracion asc, tabla.hora_valoracion asc"*/;
				logger.info("SQL / cargarDiagnosticosEvolucion / "+consultaReal);
