 ...
 *
 */
public class OracleGeneracionCargosPendientesDao implements GeneracionCargosPendientesDao 
{
	/**
	...
	private final static String detalleSolProcedimientoPORTATILStr=			"SELECT  " +
																	...
																	" FROM solicitudes s " +
																		...
																		" LEFT OUTER JOIN ( select rsp1.numero_solicitud as numero_solicitud,rsp1.tipo_recargo as tipo_recargo FROM res_sol_proc rsp1 WHERE rsp1.numero_solicitud=? "+ValoresPorDefecto.getValorLimit1()+" 1 order by rsp1.fecha_grabacion desc, rsp1.hora_grabacion desc ) temprec on(temprec.numero_solicitud=s.numero_solicitud) " +
																	" WHERE " +
																		" s.numero_solicitud=?";
