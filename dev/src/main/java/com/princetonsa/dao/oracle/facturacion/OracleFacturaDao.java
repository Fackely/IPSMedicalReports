 ...
 *
 */
public class OracleFacturaDao implements FacturaDao
{
	/**
	 * 
	 * @param con
	...
	{
		String hora= ValoresPorDefecto.getSentenciaHoraActualBD();
	...
	public HashMap proponerPreFactura(Connection con, DtoFactura dtoFactura)
	{
		...
		consulta+= 	"UNION "+
						"(SELECT distinct " +
							...
							"FROM " +
								"(SELECT distinct " +
									"dca.cantidad_cargada as cantidad, dca.solicitud as sol, dca.servicio_cx as serv " +
								"FROM " +
									"det_cargos dca " +
								"INNER JOIN " +
									"sub_cuentas sc ON (sc.sub_cuenta=dca.sub_cuenta) " +
								"INNER JOIN " +
									"solicitudes_subcuenta ssc on (dca.cod_sol_subcuenta=ssc.codigo)  " +
								"WHERE " +
									...
									"AND dca.paquetizado='"+ConstantesBD.acronimoNo+"' " +///con esto solo cargamos el encabezado de ese paquete
									"AND dca.articulo is null " + 
									"AND dca.servicio_cx is not null " +
									"AND dca.cantidad_cargada>0 "+									
									"AND dca.valor_unitario_cargado >0 "+
							") tabla " +
							" WHERE tabla.serv=dc.servicio_cx " +
							") as cantidadcargo, " +
							// ****
						...
						"WHERE " +
							...
							"getnomcentrocosto(s.centro_costo_solicitado), " +
							"dc.servicio_cx, " +
							"ssc.solicitud, dc.sub_cuenta " +
							") ";
		consulta+=") tabla order by tabla.nombrecentrocosto, tabla.descarticuloservicio";*/
		/**
		davgommo
		El codigo habia sido modificado en la MT 5837, pero no servia para la MT 3463 ya que solo manejaba servicios
		**/
		String consulta =
				"SELECT * FROM(" +
				" (SELECT 1 AS tipo, " +
