 ...
 */
public class OracleConsultasBirtDao implements ConsultasBirtDao {
	 ...
	 * @return
	 */
	public String furipsAccidenteDeTransito(int factura, int codigoMedicoTratante, int codigoReclamacion) {
		return SqlBaseConsultasBirtDao.furipsAccidenteDeTransito(factura, codigoMedicoTratante,codigoReclamacion);
	}
	...
	public String consultarNumeroAutorizacion(int codigoCuenta, int codSubCuenta)
	{
				...
				"(" +
					...
					"FROM " +
						"(" +
							...
							"INNER JOIN " +
								"resp_autorizaciones respauto ON (respauto.det_autorizacion = detaut.codigo) " +
							"WHERE " +
								"cu.id = "+ codigoCuenta+ " ";
		...
		cadenaConsulta+= " UNION " +
		   					...
		   					"WHERE " +
		   						"cuent.id = "+ codigoCuenta + " ";
		
								if(codSubCuenta!=ConstantesBD.codigoNuncaValido)
									cadenaConsulta+= "AND  subc.sub_cuenta = "+ codSubCuenta +"  ";
	
		cadenaConsulta += ") tabl ORDER BY tabl.\"orden\" ) WHERE rownum = 1";
	...
	{
		//davgommo MT 3463
		// Se agregan valoraciones para que el codigo del articulo corresponda al tipo definido en el contrato
		String query = "SELECT "+
		"t.\"orden\" as orden, "+
		...
		"to_char(current_date, 'DD/MM/YYYY') AS fechaActual " +
			...
			"( "+
				"( "+
					...
					"CAST (getobtenercodigocupsserv(dc.servicio,"+ConstantesBD.codigoTarifarioCups+")AS VARCHAR(50)) "+ 					
						"ELSE "+ 
			     		"CASE WHEN c.tipo_codigo_articulos=1 THEN CAST(dc.articulo AS VARCHAR(50)) ELSE CASE WHEN c.tipo_codigo_articulos=0 THEN  cast (facturacion.getcodigocumarticulo(dc.articulo)AS VARCHAR(50)) ELSE cast (facturacion.getcodigointerfaz(dc.articulo) AS VARCHAR(50)) 	END END "+ 
			    		" END AS codigo, " +
				...
				"( "+
					...
					"getnomcentrocosto(sol.centro_costo_solicitante) AS centro_costo_solicita, "+
					"getnomcentrocosto(sol.centro_costo_solicitado) AS centro_costo_ejecuta, "+
					"CAST (getcodigoespecialidad(dc.servicio_cx)AS VARCHAR(50)) || '-' || CAST(dc.servicio_cx AS VARCHAR(50))  AS codigo, " +
					...
					"dc.articulo is null "+
					"GROUP BY dc.cargo_padre,  dc.valor_unitario_cargado,  dc.valor_unitario_recargo,  dc.valor_unitario_dcto,"+
    "dc.usuario_modifica,  dc.fecha_modifica,   dc.det_cx_honorarios,    sol.centro_costo_solicitante,    sol.centro_costo_solicitado, sol.consecutivo_ordenes_medicas,"+
    " dc.servicio_cx,    c.tipo_regimen,    sbc.ingreso,    c.nombre,    dc.codigo_factura,    tpa.codigo_asocio,    tpa.nombre_asocio"+
				") "+
				...
				"( "+
					...
					"getnomcentrocosto(sol.centro_costo_solicitante) AS centro_costo_solicita, "+
					"getnomcentrocosto(sol.centro_costo_solicitado) AS centro_costo_ejecuta, "+
					"CASE WHEN c.tipo_codigo_articulos=1 THEN CAST(dc.articulo AS VARCHAR(50)) ELSE CASE WHEN c.tipo_codigo_articulos=0 THEN  cast (facturacion.getcodigocumarticulo(dc.articulo)AS VARCHAR(50)) ELSE cast (facturacion.getcodigointerfaz(dc.articulo) AS VARCHAR(50)) 	END END AS codigo," +
	...
	{
		//davgommo MT 3463 
		// El codigo del articulo debe corresponder al definido en el campo tipo codigo en el convenio
		...
		String query =   "SELECT "+ 
	    	...
	    	"( "+
	    		"( "+
					...
					"to_char(dc.fecha_modifica,'"+ConstantesBD.formatoFechaAp+"') AS fecha_cargo, "+
					"CASE WHEN dc.servicio IS NOT NULL THEN "+ 
						"CASE WHEN sol.tipo = "+ConstantesBD.codigoTipoSolicitudProcedimiento+" AND (SELECT count(1) from sol_procedimientos WHERE numero_solicitud = dc.solicitud and portatil_asociado = dc.servicio) > 0 THEN "+
							"coalesce(getobtenercodigocupsserv((SELECT codigo_servicio_solicitado from sol_procedimientos WHERE numero_solicitud = dc.solicitud),"+ConstantesBD.codigoTarifarioCups+"),'---') "+
						" ELSE "+
							"coalesce(getobtenercodigocupsserv(dc.servicio,"+ConstantesBD.codigoTarifarioCups+"),'---') "+
						" END "+
					"ELSE "+ 
					 "CASE"+
				      " WHEN dc.servicio IS NOT NULL     THEN      CAST (getobtenercodigocupsserv(dc.servicio,0) AS VARCHAR(50))"+
				       "ELSE   CASE        WHEN c.tipo_codigo_articulos=1        THEN CAST(dc.articulo AS VARCHAR(50))"+
				          "ELSE  CASE WHEN c.tipo_codigo_articulos=0      THEN CAST (facturacion.getcodigocumarticulo(dc.articulo)AS VARCHAR(50))"+
				            "  ELSE CAST (facturacion.getcodigointerfaz(dc.articulo) AS   VARCHAR(50))"+
				           " END END   END AS codigo, "+"CASE WHEN dc.servicio IS NOT NULL THEN "+ 
	    		...
	    		") "+
	    	") t "+
	    "GROUP BY t.\"servicio_cx\", t.\"es_encabezado_cx\", t.\"fecha_cargo\",t.\"solicitud\",t.\"codigo\",t.\"codigoalterno\",t.\"descripcion\",t.\"asocio\",t.\"paquete\",t.\"ctc\",t.\"valoru\",t.\"valort\",t.\"profesional\",t.\"es_portatil\",t.\"codigo_portatil\" "+
	...
	public String consultaImprimirItemEstadoCuenta(String estadoAction,int remite,String idSubCuenta,String numeroSolicitud)	
	{
    	...
    	"( "+
    		...
    		") "+
    	") t "+
    "GROUP BY t.\"servicio_cx\", t.\"es_encabezado_cx\", t.\"fecha_cargo\",t.\"solicitud\",t.\"codigo\",t.\"codigoalterno\",t.\"descripcion\",t.\"asocio\",t.\"paquete\",t.\"ctc\",t.\"valoru\",t.\"valort\",t.\"profesional\",t.\"es_portatil\",t.\"codigo_portatil\" ,t.\"convenio\""+
    "ORDER BY t.\"solicitud\" ASC, t.\"servicio_cx\" ASC,  t.\"es_encabezado_cx\" desc  ";
    return query;
}
	...
	public String consultaImprimirItem2EstadoCuenta(String idSubCuenta)
	{
		...
		"FROM " +
			...
			") tabla GROUP BY tabla.\"tipom\",tabla.\"nomtipoc\"";
		return query;
	}
		...
		String query =  "SELECT "+ 
		    	...
		    	"( "+
		    		...
		    		"( "+
		    			...
		    			"to_char(dc.fecha_modifica,'"+ConstantesBD.formatoFechaAp+"') AS fecha_cargo, "+
		    			//"getcodigoespecialidad(dc.servicio_cx) || '-' || dc.servicio_cx  AS codigo, "+
		    			" cast(getcodigocups(dc.servicio,"+ConstantesBD.codigoTarifarioCups+")AS VARCHAR(50)) As codigo,"+
		    			"substr(UPPER(getnombreservicio(dc.servicio_cx,"+ConstantesBD.codigoTarifarioCups+")),0,28) || ' - ' || substr(tpa.nombre_asocio,0,28)  AS descripcion, "+
		    			"1 AS cantidad, "+
		    			"sum(coalesce(dc.valor_total_cargado,0)) AS total_cargo, "+
		    			"sum(coalesce(dc.valor_unitario_recargo,0)) AS total_recargo, "+
		...
		String query = "SELECT "+
				...
				"( "+
					"( "+
						...
						"((((coalesce(dc.valor_unitario_cargado,0))+(coalesce(dc.valor_unitario_recargo*dc.cantidad_cargada,0)))-(coalesce(dc.valor_unitario_dcto*dc.cantidad_cargada,0)))*(coalesce(dc.cantidad_cargada,0))) AS valort, " +
						"dc.usuario_modifica AS usuario, " +
						"CASE WHEN dc.cargo_padre IS NOT NULL THEN getobtenercodigopaquete(dc.cargo_padre) ELSE '' END AS paquete, " +
						"to_char(dc.fecha_modifica,'"+ConstantesBD.formatoFechaAp+"') AS fecha_cargo, "+
						"CASE WHEN dc.servicio IS NOT NULL THEN getnombremedico(sol.codigo_medico_responde) ELSE '' END AS profesional, "+
					...
					"( "+
						...
						"getnomcentrocosto(sol.centro_costo_solicitante) AS centro_costo_solicita, "+
						"getnomcentrocosto(sol.centro_costo_solicitado) AS centro_costo_ejecuta, "+
						"CAST (getobtenercodigocupsserv(dc.servicio_cx,"+ConstantesBD.codigoTarifarioCups+") AS VARCHAR(50)) AS codigo, " +
						" facturacion.getnomtiposervicio(dc.servicio_cx) AS grupo, "+
						"UPPER(getnombreservicio(dc.servicio_cx,"+ConstantesBD.codigoTarifarioCups+")) AS descripcion, "+
						"1 AS cantidad, "+
						"CASE WHEN dc.cargo_padre IS NULL THEN sum(coalesce(dc.valor_total_cargado,0)) ELSE 0 END AS total_cargo, "+
						...
						"dc.tipo_solicitud = "+ConstantesBD.codigoTipoSolicitudCirugia+" AND " +
						"dc.articulo is null "+
						"GROUP BY dc.cargo_padre,dc.valor_unitario_cargado,dc.valor_unitario_recargo,dc.valor_unitario_dcto,dc.usuario_modifica,dc.fecha_modifica,dc.det_cx_honorarios,sol.centro_costo_solicitante,sol.centro_costo_solicitado,sol.consecutivo_ordenes_medicas,dc.servicio_cx,c.tipo_regimen,sbc.ingreso,c.nombre,dc.codigo_factura,tpa.codigo_asocio,tpa.nombre_asocio "+
					") "+
					//Detalle de los materiales especiales
					"UNION "+ 
					"( "+
						...
						"((((coalesce(dc.valor_total_cargado,0))+(coalesce(dc.valor_unitario_recargo*dc.cantidad_cargada,0)))-(coalesce(dc.valor_unitario_dcto*dc.cantidad_cargada,0)))*(coalesce(dc.cantidad_cargada,0))) AS valort, " +
						"dc.usuario_modifica AS usuario, " +
						"CASE WHEN dc.cargo_padre IS NOT NULL THEN getobtenercodigopaquete(dc.cargo_padre) ELSE '' END AS paquete, " +
						"to_char(dc.fecha_modifica,'"+ConstantesBD.formatoFechaAp+"') AS fecha_cargo, "+
						"'' AS profesional, "+
						...
						"FROM " +
							"( " +
								"SELECT " +
									...
									"f.\"codigo_lote\", " +
									"f.\"numero_conteo\" as numero_conteo, " +
									"f.codigo_dapa, " +
	...
	public String reportePagosCarteraPaciente(String fechaIni, String fechaFin,
			...
			String tipoPeriodo)
	{
		return SqlBaseConsultasBirtDao.reportePagosCarteraPaciente(fechaIni, fechaFin,
				anioIni, anioFin, centroAtencion, tipoDoc,
				tipoPeriodo);
	}
