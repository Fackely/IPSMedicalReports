 ...
 */
public class OracleProgramasPYPDao implements ProgramasPYPDao 
{
	/**
	...
	private static final String consultarProgramasExistentesPacienteStr = " SELECT "+ 
		...
		"CASE WHEN ppp.motivo_finalizacion IS NULL THEN " +
			"'' " +
		"ELSE " +
			"ppp.motivo_finalizacion || ' (usuario: ' || ppp.usuario_finaliza || ', fecha: ' || to_char(ppp.fecha_final,'DD/MM/YYYY') ||', hora: ' || ppp.hora_final || ')' " +
	...
	{
		campos.put("secuencia","seq_prog_pyp_paciente.nextval");
	...
	{
		campos.put("secuencia","seq_act_prog_pyp_pac.nextval");
	...
	{
		campos.put("secuencia","seq_acum_act_prog_pac_pyp.nextval");
