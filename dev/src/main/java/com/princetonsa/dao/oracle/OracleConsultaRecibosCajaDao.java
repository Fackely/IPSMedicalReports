 ...
 */
public class OracleConsultaRecibosCajaDao implements ConsultaReciboCajaDao
{
	 ...
	 */
	private Logger logger = Logger.getLogger(OracleConsultaRecibosCajaDao.class);
	...
	private static String cadenaConsultaAnulacionReciboCaja="SELECT " +
                                                                ...
                                                                "marc.descripcion as descmotivoanulacion," +
                                                                 /*
	                                      						 Tipo Modificacion: Segun MT3955
	                                      						 Autor: Jesús Darío Ríos
	                                      						 usuario: jesrioro
	                                      						 Fecha: 21-02-2013
	                                      						 Descripcion: CASE y TO_CHAR para las columnas arc.observaciones (llegaba null) 
	                                      						 y rc.hora (tenia 1970/01/01) por se timestamp 
                                                                 */
																"case when arc.observaciones !=' ' then  arc.observaciones else ' ' end AS observaciones,"+
                                                                "getTotalReciboCaja(arc.numero_recibo_caja,arc.institucion) as valortotalrc," +
                                                                "rc.fecha as fechaelaboracionrc," +
                                                                "to_char(rc.hora,'HH24:MI:SS') as horaelaboracionrc,"+
    ...
    {
        Connection con=OracleUtilidadesBDDao.abrirConexion();
        ...
        else
        {
            ...
            OracleUtilidadesBDDao.cerrarConexion(con);
        }
        return mapa;
    }
