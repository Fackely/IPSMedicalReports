 ...
 * de acceso a una base de datos Oracle requeridos 
 * por la clase <code>EstadoCuenta</code>
 * 
 *	@version 1.0, 5/08/2004
 */
public class OracleEstadoCuentaDao implements EstadoCuentaDao 
{
	
	/**
	 * Consulta los Codigos de la Facturas asociadas a una subcuenta con o sin solicitud
	 */
	private static String consultaCodigosFacturas="SELECT getcodigosfacturas(?,?) AS codf from dual ";

	/**
	 * Implementación del método que carga todas las
	 * cuentas del paciente en una BD Oracle
	 * 
	...
	public  ArrayList<HashMap<String, Object>> cargarTodosConvenioIngreso (Connection con, String idIngreso) 
	{
		...
		"CASE WHEN con.tipo_regimen = '"+ConstantesBD.codigoTipoRegimenParticular+"' THEN carterapaciente.getNomDeudorIngreso(sc.ingreso) ELSE con.nombre ||' - '|| coalesce(facturacion.getnumerocontrato(sc.contrato),'') END  AS nombre_convenio, "+  
		"manejopaciente.getnombreestrato(sc.clasificacion_socioeconomica) as estrato_social, "+
		...
		"manejopaciente.getnomnatpacientes(sc.naturaleza_paciente) as nombre_naturaleza," +
		...
		"facturacion.getconsecfacsubcuenta(sc.sub_cuenta,"+ConstantesBD.codigoEstadoFacturacionFacturada+") AS numerofactura " +
		"FROM MANEJOPACIENTE.sub_cuentas sc "+ 
		"INNER JOIN facturacion.convenios con ON (con.codigo=sc.convenio) " +
		"LEFT OUTER JOIN facturacion.detalle_monto mc ON (mc.detalle_codigo=sc.monto_cobro) "+
		"LEFT OUTER JOIN facturacion.tipos_monto tipm ON (mc.tipo_monto_codigo=tipm.codigo) "+ 
		"LEFT JOIN facturacion.detalle_monto_general dg ON (mc.detalle_codigo=dg.detalle_codigo) "+
		"WHERE sc.ingreso = ? "+ 
		...
		"CASE WHEN mc.porcentaje IS NULL THEN coalesce(mc.valor,0) ||'' ELSE '% ' || mc.porcentaje  END AS nombre_monto_cobro, "+
		"tipm.nombre as tipo_monto, "+ 
		"sc.naturaleza_paciente as codigo_naturaleza, "+ 
	...
	@Override
	public double consultarAbonos (Connection con, int idCuenta, int codigoPaciente, Integer idInteger) 
	{
		return SqlBaseEstadoCuentaDao.consultarAbonos(con, idCuenta, codigoPaciente, idInteger);
	}
