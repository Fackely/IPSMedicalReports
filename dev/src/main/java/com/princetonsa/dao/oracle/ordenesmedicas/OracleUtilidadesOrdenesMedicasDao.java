 ...
 * del módulo de ORDENES
 */
public class OracleUtilidadesOrdenesMedicasDao implements
		UtilidadesOrdenesMedicasDao 
{
	...
	{
		String consultarFechaValoracion = "SELECT to_char(val.fecha_valoracion,'"+ConstantesBD.formatoFechaBD+"') as fecha_valoracion from valoraciones val INNER JOIN solicitudes sol " +
		"ON(sol.numero_solicitud=val.numero_solicitud) " +
		"where " +
		"sol.cuenta=?  "+
		ValoresPorDefecto.getValorLimit1()+"1 order by val.numero_solicitud";
