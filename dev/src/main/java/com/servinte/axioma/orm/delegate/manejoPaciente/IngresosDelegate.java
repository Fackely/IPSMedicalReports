...
public class IngresosDelegate extends IngresosHome {
	...
	public ArrayList<DtoInfoIngresoPacienteControlarAbonoPacientes> cargarIngresosPorPaciente(int codPaciente, String parametroGeneralControlarAbonoPacientesXIngreso) {
		...
		if (parametroGeneralControlarAbonoPacientesXIngreso.equals(ConstantesBD.acronimoSi)){
			criteria.setProjection(Projections.projectionList()
					...
					.add( Projections.sqlProjection("getabonodisponible(this_.codigo_paciente, id) AS saldoActual", new String[]{"saldoActual"}, new Type[]{Hibernate.DOUBLE})));
	...
	public List<DtoInfoIngresoTrasladoAbonoPaciente> obtenerIngresosParaTrasladoPorPaciente (int codPaciente, boolean listarPorIngreso)
	{
		...
		if(listarPorIngreso)
		{
			...
			projectionList.add( Projections.sqlProjection("getabonodisponible(this_.codigo_paciente, id) AS abonoDisponible", new String[]{"abonoDisponible"}, new Type[]{Hibernate.DOUBLE}));
		...
		else
		{
			...
			projectionList.add( Projections.property("ingreso.estado")				,"estadoIngreso" );
			projectionList.add( Projections.property("pac.codigoPaciente")			,"codigoPaciente" );
			projectionList.add( Projections.sqlProjection("getabonodisponible(this_.codigo_paciente, null) AS abonoDisponible", new String[]{"abonoDisponible"}, new Type[]{Hibernate.DOUBLE}));
		}
	...
	public DtoInfoIngresoTrasladoAbonoPaciente obtenerUltimoIngresoAbiertoPaciente (int codPaciente, boolean parametroManejoEspecialInstiOdontologicas)
	{
		...
		projectionList.add( Projections.property("centroAtencion.consecutivo")	,"centroAtencion")
			...
			.add( Projections.sqlProjection("getabonodisponible(this_.codigo_paciente, null) AS abonoDisponible", new String[]{"abonoDisponible"}, new Type[]{Hibernate.DOUBLE}));
	...
	public List<DtoIngresos> consultarIngresosConsultaExternaPorFechaHoraEstadoCita(int codigoPaciente, Date fecha, String hora, int estadoCita){
		...
		try{
			...
			query.setParameter("codigoPaciente", codigoPaciente, Hibernate.INTEGER);
			query.setParameter("fecha", fecha, Hibernate.DATE);
			query.setParameter("hora", hora, Hibernate.STRING);
			query.setParameter("estadoCita", estadoCita, Hibernate.INTEGER);
			query.setParameter("viaIngreso", ConstantesBD.codigoViaIngresoConsultaExterna, Hibernate.INTEGER);
			query.addScalar("idIngreso", Hibernate.INTEGER);
			query.addScalar("consecutivo", Hibernate.STRING);
			query.addScalar("fechaIngreso", Hibernate.DATE);
			query.addScalar("horaIngreso", Hibernate.STRING);
			query.addScalar("codViaIngreso", Hibernate.INTEGER);
			query.addScalar("nomViaIngreso", Hibernate.STRING);
			query.addScalar("codCentroAtencion", Hibernate.INTEGER);
			query.addScalar("nomCentroAte", Hibernate.STRING);
			query.addScalar("codCuenta", Hibernate.INTEGER);
	...
	public InfoDatos consultarUltimoDiagnosticoRespuestaSolicitudes(int codigoIngreso, int tipoSolicitud){
		...
		try{
			...
			SQLQuery query = sessionFactory.getCurrentSession().createSQLQuery(consulta);
			query.setParameter("codigoIngreso", codigoIngreso, Hibernate.INTEGER);
			query.setParameter("tipoSolicitud", tipoSolicitud, Hibernate.INTEGER);
			query.addScalar("acronimoDiag", Hibernate.STRING);
			query.addScalar("tipoCieDiag", Hibernate.INTEGER);
			query.addScalar("nombreDiag", Hibernate.STRING);
			List<Object[]> resultado=query.list();
			for(Object[] result:resultado){
