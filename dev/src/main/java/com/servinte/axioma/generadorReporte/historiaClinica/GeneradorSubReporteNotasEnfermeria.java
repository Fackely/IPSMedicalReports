...
public class GeneradorSubReporteNotasEnfermeria {
	...
	public   JasperReportBuilder notasEnfermiria(DtoImpresionHistoriaClinica dto
		...
		VerticalListBuilder list = cmp.verticalList(); //Lista vertical
		
		JasperReportBuilder notasEnfermiria = report(); //SubReporte notas enfermería
...
//		componentBuilder = cmp.verticalList(tituloSeccion);
		...
		list.add(cmp.subreport(notasEnfermiriaTable(dto, usuario, paciente)));
...
//		.build();
		...
		notasEnfermiria.addSummary(list);
		notasEnfermiria.build();
